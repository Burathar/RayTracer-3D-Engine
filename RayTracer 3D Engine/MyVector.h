...
public:
	...
	double get_vect_x() const { return x_; }
	double get_vect_y() const { return y_; }
	double get_vect_z() const { return z_; }
	...
	my_vector negative() const
	{
		return my_vector(-x_, -y_, -z_);
	}

	double dot_product(my_vector v) const
	{
		return x_ * v.get_vect_x() + y_ * v.get_vect_y() + z_ * v.get_vect_z();
	}

	my_vector cross_product(my_vector v) const
	{
		return my_vector(
			y_ * v.get_vect_z() - z_ * v.get_vect_y(),
			z_ * v.get_vect_x() - x_ * v.get_vect_z(),
			x_ * v.get_vect_y() - y_ * v.get_vect_x());
	}

	my_vector vect_add(my_vector v) const
	{
		return my_vector(x_ + v.get_vect_x(), y_ + v.get_vect_y(), z_ + v.get_vect_z());
	}

	my_vector vect_mult(double scalar) const
	{
